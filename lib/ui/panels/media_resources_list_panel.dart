...
class _MediaResourcesListTopBar extends ConsumerWidget {
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              CustomIconButton(
                  ...
                  onPressed: () async {
                    final result = await showDialog(
                        context: context,
                        builder: (context) {
                          return MediaResourcesSortDialog(
                            sort: ref.watch(
                                settingsProvider.select((state) => state.sort)),
                          );
                        });
                    if (result != null) {
                      ref.read(settingsProvider.notifier).setSort(result);

                      ref
                          .read(mediaResourcesStateProvider.notifier)
                          .sortMediaResources(result);
                    }
                  },
              ...
              CustomIconButton(
                  iconData: ref.watch(mediaResourcesStateProvider
                          .select((state) => state.isMultipleSelection))
                      ? Icons.keyboard_return
                      : Icons.checklist_rtl,
                  onPressed: () {
                    if (ref.watch(mediaResourcesStateProvider
                        .select((state) => state.isMultipleSelection))) {
                      ref
                          .read(mediaResourcesStateProvider.notifier)
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    extension == 'MP4'
                                        ...
                                        : const SizedBox(),
                                    extension == 'MP4'
                                        ? (mediaResource as VideoResource)
                                                .isHevc
                                            ? SizedBox(
                                                width: DesignValues
                                                    .ultraSmallPadding)
...
class MediaResourcesListPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final mediaResourcesLength = ref.watch(
        mediaResourcesStateProvider.select((value) => value.resources.length));
    return Column(
      mainAxisAlignment: MainAxisAlignment.start,
      children: [
        const _MediaResourcesListTopBar(),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _MediaResourceListWidget(
                                ...
                                isSelected: isMultipleSelection
                                    ? ref.watch(
                                        mediaResourcesStateProvider.select(
